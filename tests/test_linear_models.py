...
def test_linear_regression():
    X = np.array([[1], [2], [3], [4]])
    y = np.array([2, 4, 6, 8])  # perfect line: slope=2

    model1 = LinearRegression(use_intercept=True, coef=None)
    model1.fit(X, y)
    preds1 = model1.predict(X)
    # Expect slope ≈ 2, intercept ≈ 0
    assert np.allclose(model1.coef[0], 0, atol=1e-6)   # intercept
    assert np.allclose(model1.coef[1], 2, atol=1e-6)   # slope
    ...
    assert np.allclose(model2.coef[0], 2, atol=1e-6)
